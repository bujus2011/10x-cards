  ...
  return (
    <div className="container mx-auto py-8 px-4">
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-foreground mb-2">{t("pages.myFlashcards.title")}</h1>
          <p className="text-muted-foreground">{t("pages.myFlashcards.noFlashcardsDescription")}</p>
        </div>
        ...
        <div className="space-y-6">
          {error && (
            <div className="bg-destructive/10 border border-destructive/50 rounded-lg p-4 flex gap-3" role="alert">
              ...
              <div>
                <h3 className="font-medium text-destructive">{t("components.errorNotification.title")}</h3>
                ...
                >
                  {t("components.errorNotification.retry")}
                </Button>
              </div>
            </div>
          )}
          ...
          {searchStats.total > 0 && (
            <div className="space-y-4">
              <div className="relative">
                ...
                />
                <Input
                  placeholder={t("pages.myFlashcards.searchPlaceholder")}
                  value={searchQuery}
              ...
              <div className="text-sm text-muted-foreground" aria-live="polite">
                {t("pages.myFlashcards.flashcardCount", {
                  filtered: searchStats.filtered,
                  total: searchStats.total,
                })}
                {searchStats.hasQuery && ` (${t("pages.myFlashcards.filtered")})`}
              </div>
            </div>
          )}
          ...
          {searchStats.filtered === 0 && searchStats.total === 0 && (
            <div className="text-center py-12">
              <div className="text-lg font-medium text-muted-foreground mb-2">
                {t("pages.myFlashcards.noFlashcards")}
              </div>
              <p className="text-sm text-muted-foreground">{t("pages.myFlashcards.noFlashcardsDescription")}</p>
            </div>
          )}
          ...
          {searchStats.filtered === 0 && searchStats.total > 0 && (
            <div className="text-center py-12">
              ...
              <p className="text-sm text-muted-foreground">{t("pages.myFlashcards.searchPlaceholder")}</p>
            </div>
          )}
