...
export function useStudySession() {
    ...
    const fetchDueCards = useCallback(async (limit: number = 20): Promise<{ data: StudyCardDto[]; error?: string }> => {
        setIsLoading(true);
        try {
    ...
    const submitReview = useCallback(async (flashcardId: number, rating: Rating): Promise<{ success: boolean; error?: string; data?: SubmitReviewResponseDto }> => {
